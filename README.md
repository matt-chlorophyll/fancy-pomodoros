...
## 使用
    ...
    pomo sf                  # 同上，简写
    pomo rate --score 4 --comment "今天非常 focus"  # 给今天打分
    pomo report              # 今日复盘
...
`pomo rate --score 1..5 [--comment "..."]` 给当天打一个整体满意度评分；
同一天再次打分会覆盖前一次。评分会在 `pomo report` 里和 session 一起展示。
...
每次 session 记录到 `~/.fancy-pomodoro/sessions.json`，每日评分记录到同目录
下的 `ratings.json`（都是人类可读的 JSON）。可用环境变量 `POMO_DATA_DIR`
改变存储目录。
